...
def hrs_ot(doc, method):
    if not doc.employee or not doc.start_date:
        frappe.throw("Employee and Start Date must be specified.")

    # Determine the month start date
    ...
    if overtime_pay <= 0:
        frappe.msgprint(f"No valid overtime pay found for Employee {doc.employee} on {month_start_date}.")
        return
