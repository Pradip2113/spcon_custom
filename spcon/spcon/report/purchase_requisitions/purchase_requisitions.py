...
def get_conditions(filters, query, mr, mr_item):
	...
	if filters.get("not_purchase"):
		query = query.where(mr_item.qty != mr_item.ordered_qty)
	
	if filters.get("not_ordered"):
		query = query.having(
			(Sum(Coalesce(mr_item.stock_qty, 0)) - Sum(Coalesce(mr_item.ordered_qty, 0)) == Sum(Coalesce(mr_item.qty, 0)))
		)
	if filters.get("not_received"):  
		query = query.having(
			(Sum(Coalesce(mr_item.stock_qty, 0)) - Sum(Coalesce(mr_item.received_qty, 0)) > 0)
		)
...
def prepare_data(data, filters):
	...
	if filters.get("group_by_mr"):
		data = [material_request_map[mr] for mr in material_request_map]
	else:
		# ✅ HIDE REPEATED MATERIAL REQUEST NAME
		previous_mr = None
		# previous_date = None
		for row in data:
			if row["material_request"] == previous_mr:
				row["material_request"] = ""
			else:  
				previous_mr = row["material_request"]
