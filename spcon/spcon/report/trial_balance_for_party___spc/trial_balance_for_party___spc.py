...
def get_data(filters, show_party_name):
	...
	for party in parties:
		...
		opening_debit, opening_credit, opening_ref_name = opening_balances.get(party.name, [0, 0,0])
		row.update({"opening_debit": opening_debit, "opening_credit": opening_credit, "ref_name":opening_ref_name})

		# within period
		debit, credit,ref_name = balances_within_period.get(party.name, [0, 0,0])
...
def get_opening_balances(filters, account_filter=None):
	...
	query = (
		...
		.select(
			GL_Entry.party,GL_Entry.name.as_("opening_ref_name"),
	...
	for d in gle:
		...
		opening.setdefault(d.party, [opening_debit, opening_credit,d.opening_ref_name])
	return opening
...
def get_balances_within_period(filters, account_filter=None):
	...
	query = (
		frappe.qb.from_(GL_Entry)
		.select(
			GL_Entry.party,GL_Entry.name.as_("ref_name"),
	...
	for d in gle:
		balances_within_period.setdefault(d.party, [d.debit, d.credit,d.ref_name])
