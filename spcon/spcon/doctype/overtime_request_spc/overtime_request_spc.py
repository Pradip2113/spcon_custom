...
#*****************************************************************************************************
	...
	# 			updated_leaves = (float(existing_allocation.new_leaves_allocated or 0)+ allocate_leave)
	# 			frappe.db.set_value("Leave Allocation",existing_allocation.name,"new_leaves_allocated",updated_leaves)
	# 		else:
	...
	def update_leave_allocation(self):
		for row in self.overtime_assign_leave:
			...
			# 🔍 check existing allocation (draft + submitted dono)
			...
			if existing_allocation:
				# ✅ UPDATE
				updated_leaves = (
					float(existing_allocation.new_leaves_allocated or 0)
					+ allocate_leave
				)
